  ...
  if (isProduction) {
    ...
    if (versionIdMatch) {
      ...
      if (previewUrl) {
        ...
        console.log(
          `🔗 Preview URL: https://${previewAlias}-${workerName}.<ACCOUNT_SUBDOMAIN>.workers.dev`
        );
        console.log(
          "   (Check Cloudflare dashboard or 'wrangler versions list' for exact URL)"
        );
        console.log(
          "   The account subdomain can be found in your Cloudflare dashboard"
        );
      }
      console.log("=".repeat(70));
    } else {
      ...
      console.log(
        `   Expected format: https://${previewAlias}-${workerName}.workers.dev`
      );
    }
  }
} catch (error) {
