...
import { parseDemoResponse } from "@/domain/live-demo";
import type { DemoResponse, JsonDemoId } from "@/domain/live-demo";
import { TURNSTILE_ACTIONS } from "@/domain/turnstile";
...
const DEMOS: readonly DemoDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Read metadata for one fixed object with no public bucket or uploads.",
...
const DemoIcon = ({ id }: { readonly id: JsonDemoId }) => {
  switch (id) {
    case "d1": {
...
const turnstileRequest = (token: string): RequestInit => ({
...
const DemoCard = ({ demo }: { readonly demo: DemoDefinition }) => {
  ...
  const prove = async (token: string): Promise<void> => {
    ...
    try {
      ...
      if (
        ...
        setState({
          _tag: "error",
          message: "The proof response was malformed.",
        });
        return;
      }
  ...
  return (
    <article className="demo-card human-proof-card">
      ...
      <p className="demo-description">
        Generate a single-use token, verify it server-side, then enforce its
        action and hostname.
      </p>
      <div className="demo-console" aria-live="polite">
        {state._tag === "idle" ? (
          <div className="demo-empty">
            ...
            <p>No challenge has run.</p>
          </div>
        ) : null}
        {state._tag === "loading" ? (
          <div className="demo-loading">
            <ArrowClockwise aria-hidden="true" weight="bold" />
            <span>Checking Siteverify</span>
          </div>
        ) : null}
        {state._tag === "error" ? (
          <div className="demo-error" role="alert">
...
const ImagesDemo = () => {
  ...
  const run = async (token: string): Promise<void> => {
    ...
    try {
      ...
      setState({
        _tag: "error",
        message: "The image transform is unavailable right now.",
      });
    }
  };
  ...
  return (
    <article className="demo-card demo-card-wide">
      <div className="demo-card-heading">
        ...
        <div>
          <p>Images + Static Assets</p>
          <h3>Transform an image at the edge</h3>
        </div>
      </div>
      <p className="demo-description">
        Choose an allow-listed width. Images transforms the bundled source asset
        and returns WebP directly from the Worker.
      </p>
      ...
      <div className="image-demo-layout">
        <div className="image-demo-controls">
          ...
          <fieldset
            className="image-width-options"
            aria-label="Image output width"
          >
          ...
          <ul>
            <li>Fixed source asset</li>
            <li>Three accepted widths</li>
            <li>100 transformations per day</li>
          </ul>
          <TurnstileActionButton
        ...
        <div className="image-demo-output" aria-live="polite">
          {state._tag === "idle" ? (
            <div className="image-placeholder">
              <ImageSquare aria-hidden="true" weight="thin" />
              <span>OUTPUT WAITS HERE</span>
            </div>
          ) : null}
          {state._tag === "loading" ? (
            <div className="image-placeholder">
              <ArrowClockwise aria-hidden="true" weight="bold" />
              <span>Transforming at the edge</span>
            </div>
          ) : null}
          {state._tag === "error" ? (
            <div className="image-placeholder demo-error" role="alert">
              <WarningCircle aria-hidden="true" weight="fill" />
...
export const LiveLab = () => (
  <TurnstileProvider>
    <section
      className="section live-lab-section"
      id="live-lab"
      aria-labelledby="live-lab-title"
    >
      <MotionReveal className="section-heading live-lab-heading">
        ...
        <p>
          Every action below invokes the deployed Worker and names the products
          that did the work. A fresh Turnstile token guards every request.
        </p>
        <div className="free-contract">
          <LockKey aria-hidden="true" weight="duotone" />
          <div>
            <strong>THE FREE CONTRACT</strong>
            <span>
              Allow-listed inputs, human checks, private storage, daily hard
              stops.
            </span>
          </div>
        </div>
      ...
      <div className="demo-grid">
        ...
        {DEMOS.map((demo, index) => (
          <MotionReveal delay={(index + 1) * 0.05} key={demo.id}>
            <DemoCard demo={demo} />
          </MotionReveal>
        ))}
        <MotionReveal className="demo-grid-wide" delay={0.25}>
          <ImagesDemo />
        </MotionReveal>
      </div>
    </section>
